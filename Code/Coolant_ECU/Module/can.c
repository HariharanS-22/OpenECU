...
void CAN1_Init(){
	...
	CAN1->sFilterRegister[0].FR1 = ECU3_Shell << 5;
...
void CAN1_TxMsg(uint8_t* tx_msg, uint8_t DLC){
	...
	CAN1->sTxMailBox->TIR 	= (ECU1_Coolant << 21);		//RTR - bit 1 | IDE - bit 2
...
void CAN1_LoopBack(){
	...
	//Loopback Enable
	CAN1->BTR |= BTR_LBKM;
	CAN1->sFilterRegister[0].FR1 |= (ECU1_Coolant << 5) << 16 ;
